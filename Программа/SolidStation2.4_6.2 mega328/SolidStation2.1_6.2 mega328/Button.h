...
class Button {
	...
	boolean flagPress;    // признак кнопка в нажатом состо€нии (сигнал в низком)
	boolean flagClick;    // признак клика кнопки (фронт)
	//void  scanState();    // метод проверки ожидание стабильного состо€ни€ сигнала
	void  filterAvarage(); // метод фильтрации сигнала по среднему значению
	//void setPinTime(byte pin, byte timeButton); // метод установки номера вывода и времени (числа) подтверждени€
	//byte digitalRead(byte pin_);
	private:
	byte  _buttonCount;    // счетчик времени фильтрации
	byte _timeButton;      // врем€ фильтрации
